...
function renderMessages(mensagens) {
    ...
    mensagens.forEach(mensagem => {
        ...
        const isOperadorFaturamento = mensagem.operador === 'FATURAMENTO';
        
        // Na LOGISTICA: LOGISTICA à direita, outras à esquerda
        let messageClass = isOperadorFaturamento ? 'message-faturamento' : 'message-outro';
...
async function sendMessage() {
    ...
    try {
        ...
        const response = await fetch(`/api/chat-mensagens/chat/${selectedChatId}?operador=FATURAMENTO&mensagem=${encodeURIComponent(message)}`, {
            method: 'POST',
            headers: {
